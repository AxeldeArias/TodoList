...
const filterByState = (state: TaskItem['state']) => (task: TaskItem) => {
...
const TaskSummary = ({ containerStyle }: TaskSummaryProps) => {
  ...
  const hoursCompleted = tasks.filter(filterByState('COMPLETED')).reduce(sumEstimate, 0)
  const hoursInProgress = tasks.filter(filterByState('IN_PROGRESS')).reduce(sumEstimate, 0)
  const hoursPlanned = tasks.filter(filterByState('PLANNED')).reduce(sumEstimate, 0)
